...
namespace RobotWar.Domain
{
    ...
    {
        public WriteOnce<ArenaCoordinates> ArenaCoordinates { get; private set; }
        ...
        public void AddRobot(int x, int y, CompassPoint c)
        {
            ...
            var evt = new RobotAdded(Id, Version, x, y, EnumEx.MapByStringValue<CompassPoint, Contracts.CompassPoint>(c));
            RaiseDomainEvent(evt);
        }
        ...
        public void RotateRobot(Rotation rotation)
        {
            ...
            var evt = new RobotRotated(Id, Version, EnumEx.MapByStringValue<CompassPoint, Contracts.CompassPoint>(newCompassPoint));
            RaiseDomainEvent(evt);
        }
        ...
        public void MoveRobot()
        {
            ...
            var evt = new RobotMoved(Id, Version, robot.Coordinates.X, robot.Coordinates.Y, EnumEx.MapByStringValue<CompassPoint, Contracts.CompassPoint>(robot.CompassPoint));
            RaiseDomainEvent(evt);
        }
        ...
        {
            Robot = Option.Some(Domain.Robot.Create(RobotCoordinates.Create(evt.XPosition, evt.YPosition), EnumEx.MapByStringValue<Contracts.CompassPoint, CompassPoint>(evt.CompassPoint)));
        }

        private void Apply(RobotRotated evt)
        {
            Robot = Option.Some(Robot.Value.UpdateCompassPoint(EnumEx.MapByStringValue<Contracts.CompassPoint, CompassPoint>(evt.CompassPoint)));
        }
